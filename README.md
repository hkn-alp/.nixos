...
This system is managed via custom bash commands embedded in `modules/tier1/core/nixos-bash.nix`. It enforces a strict "Local-First" rule: changes are built and applied locally, and *only* pushed to GitHub if the build succeeds.
...
This configuration universally applies GameMode and specific input Udev rules (like 8BitDo support) via `modules/tier1/gaming/system-gaming.nix`.
