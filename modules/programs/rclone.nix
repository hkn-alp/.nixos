{ pkgs, ... }: {
  ...
  systemd.user.services.rclone-gdrive-mount = {
    ...
    serviceConfig = {
      ...
      ExecStart = "${pkgs.bash}/bin/bash -c 'PATH=/run/wrappers/bin:$PATH exec ${pkgs.rclone}/bin/rclone mount gdrive:/Main %h/gdrive-main --vfs-cache-mode full --vfs-cache-max-age 1000h --vfs-cache-max-size 20G'";
